...
object PrepDB {
    ...
    fun initCategoriesTableRevised(db: SupportSQLiteDatabase) {
        ...
        for (i in 0..15) {
            ...
            values1.put(CategoryDBAdapter.COL_CODE, list[i].code)
            values1.put(CategoryDBAdapter.COL_COLOR, 0)
            values1.put(CategoryDBAdapter.COL_SIGNIFICANCE, 0)
            values1.put(CategoryDBAdapter.COL_DRAWABLE, list[i].drawable)
...
//            values1.put(CategoryDBAdapter.COL_VAL3, 0);
            ...
            val values2 = ContentValues()
            values2.put(CategoryLanDBAdapter.COL_CODE, list[i].code)
            values2.put(CategoryLanDBAdapter.COL_ENG, list[i].eng)
            values2.put(CategoryLanDBAdapter.COL_SPA, list[i].spa)
            values2.put(CategoryLanDBAdapter.COL_FRA, list[i].fra)
            values2.put(CategoryLanDBAdapter.COL_ITA, list[i].ita)
            values2.put(CategoryLanDBAdapter.COL_JPN, list[i].jpn)
            values2.put(CategoryLanDBAdapter.COL_ARA, list[i].ara)
            values2.put(CategoryLanDBAdapter.COL_HIN, list[i].hin)
            values2.put(CategoryLanDBAdapter.COL_IND, list[i].ind)
            values2.put(CategoryLanDBAdapter.COL_KOR, list[i].kor)
            values2.put(CategoryLanDBAdapter.COL_POL, list[i].pol)
            values2.put(CategoryLanDBAdapter.COL_POR, list[i].por)
            values2.put(CategoryLanDBAdapter.COL_RUS, list[i].rus)
            values2.put(CategoryLanDBAdapter.COL_TUR, list[i].tur)
            values2.put(CategoryLanDBAdapter.COL_VIE, list[i].vie)
            values2.put(CategoryLanDBAdapter.COL_Hans, list[i].hans)
            values2.put(CategoryLanDBAdapter.COL_Hant, list[i].hant)
            //            db.insertOrThrow(CategoryLanDBAdapter.TABLE_NAME, null, values2);
            db.insert(CategoryLanDBAdapter.TABLE_NAME, OnConflictStrategy.REPLACE, values2)
            val values3 = ContentValues()
            values3.put(CategoryDspDBAdapter.COL_LOCATION, list[i].location)
            values3.put(CategoryDspDBAdapter.COL_CODE, list[i].code)
    ...
    fun addMoreCategories(db: SupportSQLiteDatabase) {
        ...
        val query = "UPDATE " + CategoryDBAdapter.TABLE_NAME +
