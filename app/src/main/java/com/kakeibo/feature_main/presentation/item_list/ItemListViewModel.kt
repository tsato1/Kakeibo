...
class ItemListViewModel @Inject constructor(
    ...
    private var recentlyDeletedItemEntity: DisplayedItem? = null
        ...
        when (event) {
            is ItemListEvent.DeleteItem -> {
                viewModelScope.launch {
                    itemUseCases.deleteItemUseCase(event.displayedItem)
                    recentlyDeletedItemEntity = event.displayedItem
                }
            }
            is ItemListEvent.RestoreItem -> {
                viewModelScope.launch {
                    itemUseCases.insertItemUseCase( // todo catch exception
                        recentlyDeletedItemEntity ?: return@launch
                    )
                    recentlyDeletedItemEntity = null
                }
            }
        }
    }
