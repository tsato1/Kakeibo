...
class DataRepository private constructor(
    ...
    init {
        ...
        subscriptions.addSource(billingClientLifecycle.purchases) { purchases ->
            // We only need to update the database if the isLocalPurchase field needs to change.
            subscriptions.value?.let { subscriptions ->
    ...
    private fun acknowledgeRegisteredPurchaseTokens(
        remoteSubscriptions: List<Subscription>
    ) {
    ...
    private fun mergeSubscriptionsAndPurchases(
        ...
        return ArrayList<Subscription>().apply {
            if (purchases != null) {
